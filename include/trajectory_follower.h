...
public:
    ...
    TrajectoryFollower(MotorDriver &motorDriver) : motorDriver_(motorDriver){};
    ...
    // std::array<std::pair<float, float>, joint_count_> joint_limits;
    ...
    /* 以下变量同时在中断和主函数中使用 */
    volatile bool runing_ = false; // 当前工作状态
...
//    }
    ...
    void _set_motor_positions(std::array<float, joint_count_> &positions) {
        for (int id = 1; id <= joint_count_; id++) {
            motorDriver_.set_position(id, positions[id]);
        }
    }
    ...
    void on_sync_write() {
        if (runing_) {
            ...
            if (next_point_index_ == 0) {
                ...
                // 线性插值
                if (linear_interpolate(current_trajectory_.points[next_point_index_ - 1],
                                       current_trajectory_.points[next_point_index_], desired_point_) != 0) {
                }
                // 设定电机状态
                _set_motor_positions(desired_point_.positions);
            } else {
            ...
            // 打印状态
            if (last_print_time == 0 || Time::now().sec - last_print_time > 1) {
                last_print_time = Time::now().sec;
                auto &p = desired_point_;
                printf("[WORKING]: %zu/%zu %.3f sec [", next_point_index_,
                       current_trajectory_.points_length, (now - time_start_) / 1e9f);
                for (int i = 0; i < joint_count_; i++) {
                    if (i > 0)
                        printf(",");
                    printf("%.2f", R2D(p.positions[i]));
    ...
    // 若轨迹值非法, 则禁用电机, 返回-3
    int set_goal(const GoalHandle& gh) {
//        if (emergency_stop()) {
//            // 急停状态
...
//        }
        ...
        printf("Goal accepted, start trajectory follow\n");
